-- Faster materialized version of landmark and outcome construction for PostgreSQL.
...
DROP TABLE IF EXISTS saki_dynamic.kdigo_window;
CREATE TABLE saki_dynamic.kdigo_window AS
SELECT
    l.stay_id,
    l.landmark_time,
    k.charttime,
...
CREATE INDEX IF NOT EXISTS idx_saki_kdigo_window_stay_landmark_time
    ON saki_dynamic.kdigo_window (stay_id, landmark_time, charttime);

DROP TABLE IF EXISTS saki_dynamic.rrt_window;
CREATE TABLE saki_dynamic.rrt_window AS
SELECT
    l.stay_id,
    l.landmark_time,
    r.charttime,
...
CREATE INDEX IF NOT EXISTS idx_saki_rrt_window_stay_landmark_time
    ON saki_dynamic.rrt_window (stay_id, landmark_time, charttime);

DROP TABLE IF EXISTS saki_dynamic.current_kdigo;
CREATE TABLE saki_dynamic.current_kdigo AS
SELECT DISTINCT ON (stay_id, landmark_time)
    stay_id,
    landmark_time,
    aki_stage AS current_kdigo
FROM saki_dynamic.kdigo_window
WHERE charttime <= landmark_time
ORDER BY stay_id, landmark_time, charttime DESC;

DROP TABLE IF EXISTS saki_dynamic.prior_kdigo;
CREATE TABLE saki_dynamic.prior_kdigo AS
SELECT
    stay_id,
    landmark_time,
...
DROP TABLE IF EXISTS saki_dynamic.prior_rrt;
CREATE TABLE saki_dynamic.prior_rrt AS
SELECT
    stay_id,
    landmark_time,
    MAX(rrt) AS prior_rrt
FROM saki_dynamic.rrt_window
WHERE charttime <= landmark_time
GROUP BY stay_id, landmark_time;

DROP TABLE IF EXISTS saki_dynamic.future_kdigo;
CREATE TABLE saki_dynamic.future_kdigo AS
SELECT
    stay_id,
    landmark_time,
...
DROP TABLE IF EXISTS saki_dynamic.future_rrt;
CREATE TABLE saki_dynamic.future_rrt AS
SELECT
    stay_id,
    landmark_time,
