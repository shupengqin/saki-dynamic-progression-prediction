...
-- Version with time-stamped CRRT events from mimiciii_derived.crrt_durations.
-- Outcome: KDIGO stage progression within 48h or new CRRT initiation within 48h.
...
SELECT
    ...
    id.gender,
    CASE WHEN id.admission_age > 89 THEN 90 ELSE id.admission_age END AS age,
...
SELECT
    ...
    1 AS rrt
FROM mimiciii_derived.crrt_durations
...
WHERE ce.valuenum IS NOT NULL
  AND (
      ...
      OR LOWER(di.label) LIKE '%o2 saturation%'
      OR LOWER(di.label) LIKE '%blood pressure systolic%'
      OR LOWER(di.label) LIKE '%blood pressure diastolic%'
      OR LOWER(di.label) LIKE '%blood pressure mean%'
      OR LOWER(di.label) LIKE '%bp [systolic]%'
      OR LOWER(di.label) LIKE '%bp [diastolic]%'
      OR LOWER(di.label) LIKE '%bp mean%'
      OR LOWER(di.label) LIKE '%nbp [systolic]%'
      OR LOWER(di.label) LIKE '%nbp [diastolic]%'
      OR LOWER(di.label) LIKE '%nbp mean%'
      OR LOWER(di.label) LIKE '%art bp systolic%'
      OR LOWER(di.label) LIKE '%art bp diastolic%'
      OR LOWER(di.label) LIKE '%left radial map%'
      OR LOWER(di.label) LIKE '%radial map%'
      OR LOWER(di.label) LIKE '%femoral abp mean%'
  )
  AND LOWER(di.label) NOT LIKE '%alarm%'
  AND LOWER(di.label) NOT LIKE '%pulmonary%'
  AND LOWER(di.label) NOT LIKE '%pap [%'
  AND LOWER(di.label) NOT LIKE '%cuff pressure%'
  AND LOWER(di.label) NOT LIKE '%unloading%';
...
SELECT
    ...
    AVG(CASE WHEN LOWER(label) LIKE '%systolic%' THEN valuenum END) AS sbp_mean,
    MIN(CASE WHEN LOWER(label) LIKE '%systolic%' THEN valuenum END) AS sbp_min,
    AVG(CASE WHEN LOWER(label) LIKE '%diastolic%' THEN valuenum END) AS dbp_mean,
    MIN(CASE WHEN LOWER(label) LIKE '%diastolic%' THEN valuenum END) AS dbp_min,
    AVG(CASE WHEN LOWER(label) LIKE '%mean%' OR LOWER(label) LIKE '%map%' THEN valuenum END) AS map_mean,
    MIN(CASE WHEN LOWER(label) LIKE '%mean%' OR LOWER(label) LIKE '%map%' THEN valuenum END) AS map_min,
...
DROP TABLE IF EXISTS saki_mimiciii.modeling_dataset_with_crrt;
CREATE TABLE saki_mimiciii.modeling_dataset_with_crrt AS
SELECT
    l.subject_id,
    l.hadm_id,
    l.stay_id,
...
SELECT landmark_hour, COUNT(*) AS n_rows, COUNT(DISTINCT stay_id) AS n_stays, AVG(aki_progression_48h::numeric) AS event_rate, AVG(future_rrt::numeric) AS future_crrt_rate
FROM saki_mimiciii.modeling_dataset_with_crrt
