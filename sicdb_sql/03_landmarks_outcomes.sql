-- SICdb landmark and future 48-hour AKI progression outcome.

DROP TABLE IF EXISTS saki_sicdb.landmarks_raw;
CREATE TABLE saki_sicdb.landmarks_raw AS
SELECT *, saki_onset_offset + 86400 AS landmark_offset, 24 AS landmark_hour
FROM saki_sicdb.saki_onset
UNION ALL
SELECT *, saki_onset_offset + 172800 AS landmark_offset, 48 AS landmark_hour
FROM saki_sicdb.saki_onset
UNION ALL
SELECT *, saki_onset_offset + 259200 AS landmark_offset, 72 AS landmark_hour
FROM saki_sicdb.saki_onset;

DROP TABLE IF EXISTS saki_sicdb.landmark_status;
CREATE TABLE saki_sicdb.landmark_status AS
WITH current_kdigo AS (
    SELECT
        ...
        (
            SELECT k2.aki_stage
            FROM saki_sicdb.kdigo_timeline k2
            WHERE k2.stay_id = l.stay_id
              AND k2.chart_offset <= l.landmark_offset
            ORDER BY k2.chart_offset DESC
            LIMIT 1
        ) AS current_kdigo
    FROM saki_sicdb.landmarks_raw l
    LEFT JOIN saki_sicdb.kdigo_timeline k
...
future_kdigo AS (
    ...
    LEFT JOIN saki_sicdb.kdigo_timeline k
        ON l.stay_id = k.stay_id
       AND k.chart_offset > l.landmark_offset
       AND k.chart_offset <= l.landmark_offset + 172800
    GROUP BY l.stay_id, l.landmark_offset
),
future_rrt AS (
    SELECT
    ...
    LEFT JOIN saki_sicdb.rrt_events r
        ON l.stay_id = r.stay_id
       AND r.chart_offset > l.landmark_offset
       AND r.chart_offset <= l.landmark_offset + 172800
    GROUP BY l.stay_id, l.landmark_offset
)
SELECT
    l.*,
...
DROP TABLE IF EXISTS saki_sicdb.landmarks;
CREATE TABLE saki_sicdb.landmarks AS
SELECT *
FROM saki_sicdb.landmark_status
WHERE landmark_offset <= icu_outtime
  AND landmark_offset + 172800 <= icu_outtime
